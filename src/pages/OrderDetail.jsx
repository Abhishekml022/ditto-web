...
import { formatDate, numToWords, fmtDate, dateToWords, addMonths } from '../utils'
...
function buildAmenitiesString(data) {
  return [
    ...(data.amenities_list || []),
  ...
  return (
    <div className="preview-container">
      ...
      <div className="preview-details-board">
        ...
        <div className="preview-item"><div className="preview-label">Rent Purpose</div><div className="preview-val">{data.rent_purpose || '—'}</div></div>
        {!isFlat && <>
          <div className="preview-item"><div className="preview-label">Building Type</div><div className="preview-val">{data.building_type || '—'}</div></div>
        ...
        <div className="preview-item"><div className="preview-label">Amenities</div><div className="preview-val">{data.amenities || '—'}</div></div>
      </div>
...
export default function OrderDetail() {
  ...
  const config = FORMAT_CONFIGS[fmt] || FORMAT_CONFIGS['english-standard']
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'orders', id), {
        amenities,
        ...getCommonFields(editData),
        ...getSpecificFields(fmt, editData),
      })
  ...
  const handleDownload = async () => {
    ...
    try {
      const tplUrl = FORMAT_CONFIGS[fmt]?.templateUrl
      const amenities = buildAmenitiesString(order)
      ...
      saveAs(
        ...
        `Rental_Agreement_${order.tenant_name}.docx`
      )
    } catch (err) {
      console.error(err)
    } finally {
  ...
  // ── Render step for edit mode (shared components pre-filled with editData) ──
